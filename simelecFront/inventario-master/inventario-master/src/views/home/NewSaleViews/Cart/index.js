...
import { resetCart, addSale } from 'src/redux/slices/product';
...
function Cart({
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setErrors, setSubmitting }) => {
      ...
      console.log(values);
      const response = await dispatch(addSale(values));
      if(response) {
        enqueueSnackbar("Sucedió algo, genere la venta nuevamente", { variant: "warning" });
        setErrors(response.body);
        console.log('sucedió algo', response);
        setSubmitting(false);
      } else {
        setSubmitting(true);
        dispatch(resetCart());
        history.push(PATH_HOME.sales);
