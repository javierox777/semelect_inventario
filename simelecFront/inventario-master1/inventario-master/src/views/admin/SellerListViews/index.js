...
import { getSellerList, setSelectedSeller } from "src/redux/slices/seller";
...
function StudentListView() {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = sellerList.map((n) => n.name);
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - sellerList.length) : 0;
    ...
    <Page title="Vendedores | Control de inventario" className={classes.root}>
      <Container>
        ...
        <HeaderDashboard
          heading="Estudiantes"
          links={[
            { name: "Administración", href: PATH_ADMIN.root },
            { name: "Lista" },
          ]}
          action={
            <Hidden smDown>
