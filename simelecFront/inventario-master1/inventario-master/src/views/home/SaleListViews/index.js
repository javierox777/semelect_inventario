...
const TABLE_HEAD = [
  { id: "id", label: "Número de venta", alignRight: false },
...
function applySortFilter(array, comparator, query) {
  ...
  if (query) {
    array = filter(array, (sale) => {
      return sale._id.toLowerCase().indexOf(query.toLowerCase()) !== -1;
    });
    return array;
  }
...
function StudentListView() {
  ...
  const { saleList } = useSelector((state) => state.product);
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = saleList.map((n) => n._id);
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - saleList.length) : 0;
    ...
    <Page title="Vendedores | Control de inventario" className={classes.root}>
      <Container>
        <HeaderDashboard
          ...
          action={
            <Link component={RouterLink} to={PATH_HOME.sale}>
